...
def compute_multiband_connectivity(patient_id, epoch_length=1, data=data):
    ...
    for event_type, events in data['PATIENTS'][patient_id]['Events'].items():
        for event_id in events.keys():
            ...
            if(os.path.isfile('%s/%s/%s.Ictal.%s.multiband.npz'%(comp_dir,patient_id,patient_id,event_id))):
                continue
            ...
            adj_fn = os.path.join(comp_dir,patient_id,'%s.%s.%s.multiband.npz'%(patient_id,event_type,event_id))
