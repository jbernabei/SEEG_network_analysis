...
def nodal_virtual_resection(patient_id, data=data):
    ...
    for event_type, events in data['PATIENTS'][patient_id]['Events'].items():
        for event_id in events.keys():
            ...
            if os.path.exists(os.path.join(comp_dir,patient_id,'%s.%s.%s.noderes.npz'%(patient_id,event_type,event_id))):
                print os.path.join(comp_dir,patient_id,'%s.%s.%s.noderes.npz'%(patient_id,event_type,event_id))
                continue
            ...
            cres_fn = os.path.join(comp_dir,patient_id,'%s.%s.%s.noderes.npz'%(patient_id,event_type,event_id))
